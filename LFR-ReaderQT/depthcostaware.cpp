...
void DepthCostAware::calculateDepth(){
    ...
    if (focusCost.empty())
        focusCost = createFocusVolume(size_u, size_v, focus_threshold);
...
cv::Mat DepthCostAware::costAware_createCostVolume(const int size_i, const int size_j){
    ...
    DepthCostAware::calcH(pix_size, lens_size, N, c_pix, c_Mx, c_My,
                                      c_mux, c_muy, d_mu, f_M, exitPupilOffset, 20, H_data);
    ...
    double H_abs_phi_datax[9] = {1.0/F_s, 0, -c_Mx/F_s,
                              0, 1.0/F_u, -c_mux/F_u,
                              0, 0, 1};
    double H_abs_phi_datay[9] = {1.0/F_s, 0, -c_My/F_s,
                              0, 1.0/F_v, -c_muy/F_v,
                              0, 0, 1};
    double H_phi_phi_data[9] = {1, 0, 0,
                              -1.0/d_mu, 1.0/d_mu, 0,
    ...
    double H_data[25] = {Hx.at<double>(0,0), 0, Hx.at<double>(0,1),0, Hx.at<double>(0,2),
                        0, Hy.at<double>(0,0), 0, Hy.at<double>(0,1), Hy.at<double>(0,2),
                        Hx.at<double>(1,0), 0, Hx.at<double>(1,1), 0, Hx.at<double>(1,2),
                        0, Hy.at<double>(1,0), 0, Hy.at<double>(1,1), Hy.at<double>(1,2),
                        0,       0, 0,       0, 1};
    ...
    for(int k=0; k<size_k; k++){
        ...
        for(int d=0; d<size_d; d++){
            ...
            for(int j=0; j<size_j; j++){
                ...
                float zx = static_cast<float>(H.at<double>(0,0)*i_f + H.at<double>(0,4) + df*H.at<double>(0,2)*i_f + df*H.at<double>(2,4));
                float nx = static_cast<float>(H.at<double>(0,2) + df*H.at<double>(2,2));
                //float diff_k = zx / nx;
                //int k_ = k - diff_k;
                int k_ = (k-zx) / nx + 0.5f;
                //float k1 = (((k-size_k*1.0/2.0)-zx) / nx + size_k*1.0 /2.0);

                float zy = H.at<double>(1,1)*j_f + H.at<double>(1,4) + df*H.at<double>(3,1)*j_f + df*H.at<double>(3,4);
                float ny = H.at<double>(1,3) + df*H.at<double>(3,3);
                //float diff_l = zy / ny;
                //int l_ = l - diff_l;
                int l_ = (l-zy) / ny + 0.5f;
