...
void DepthStereo::calculateDepth(){
    ...
    for(int x = 2; x<4; x++){
        cv::Mat left_image = input_gray(cv::Rect(sub_w*x, sub_h*y, sub_w, sub_h)); // Note that this doesn't copy the data
        cv::Mat right_image = input_gray(cv::Rect(sub_w*(u-x), sub_h*y, sub_w, sub_h)); // Note that this doesn't copy the data
