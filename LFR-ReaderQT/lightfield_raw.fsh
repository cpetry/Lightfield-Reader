...
vec4 computeColorAt(vec2 texel_pos){
    ...
    if (x == 0 && y == 0 ){ // green
        color.g = texture2D(texture, pix_loc / tex_dim).r;
        if (view_mode >= 2){ // demosaicking
            if (demosaicking_mode == 0){ // nearest neighbour
                color.r = texture2D(texture, (pix_loc + vec2(1,0))  / tex_dim).r * ((option_wb) ? white_balance.r : 1.0f);
                color.b = texture2D(texture, (pix_loc + vec2(0,-1)) / tex_dim).r * ((option_wb) ? white_balance.b : 1.0f);
            }
            else if(demosaicking_mode == 1){ // bilinear
                color.r = (texture2D(texture, (pix_loc + vec2(1,0))   / tex_dim).r
                         + texture2D(texture, (pix_loc + vec2(-1,0))  / tex_dim).r ) / 2
                        * ((option_wb) ? white_balance.r : 1.0f);
                color.b = (texture2D(texture, (pix_loc + vec2(0,1))   / tex_dim).r
                         + texture2D(texture, (pix_loc + vec2(0,-1))  / tex_dim).r ) / 2
                        * ((option_wb) ? white_balance.b : 1.0f);
            }
        }
    }
    else if (x == 1 && y == 0 ){ // red
        color.r = texture2D(texture, pix_loc / tex_dim).r * ((option_wb) ? white_balance.r : 1.0f);
        if (view_mode >= 2){ // demosaicking
            if (demosaicking_mode == 0){ // nearest neighbour
                color.g = (texture2D(texture, (pix_loc + vec2(-1,0)) / tex_dim).r
                         + texture2D(texture, (pix_loc + vec2(0,-1)) / tex_dim).r) / 2;
                color.b = texture2D(texture, (pix_loc + vec2(-1,-1))/ tex_dim).r
                        * ((option_wb) ? white_balance.b : 1.0f);
            }
            else if(demosaicking_mode == 1){ // bilinear
                color.g = (texture2D(texture, (pix_loc + vec2(0,1))  / tex_dim).r
                         + texture2D(texture, (pix_loc + vec2(0,-1)) / tex_dim).r
                         + texture2D(texture, (pix_loc + vec2(1,0))  / tex_dim).r
                         + texture2D(texture, (pix_loc + vec2(-1,0))  / tex_dim).r ) / 4;
                color.b = (texture2D(texture, (pix_loc + vec2(-1,-1)) / tex_dim).r
                         + texture2D(texture, (pix_loc + vec2(-1,1))  / tex_dim).r
                         + texture2D(texture, (pix_loc + vec2(1,1))  / tex_dim).r
                         + texture2D(texture, (pix_loc + vec2(1,-1))  / tex_dim).r) / 4
                        * ((option_wb) ? white_balance.b : 1.0f);
            }
        }
    }
    else if (x == 0 && y == 1 ){ // blue
        color.b = texture2D(texture, pix_loc / tex_dim).r * ((option_wb) ? white_balance.b : 1.0f);
        if (view_mode >= 2){ // demosaicking
            if (demosaicking_mode == 0){ // nearest neighbour
                color.g = (texture2D(texture, (pix_loc + vec2(1,0)) / tex_dim).r
                         + texture2D(texture, (pix_loc + vec2(0,1)) / tex_dim).r) / 2;
                color.r = texture2D(texture, (pix_loc + vec2(1,1))  / tex_dim).r
                        * ((option_wb) ? white_balance.r : 1.0f);
            }
            else if(demosaicking_mode == 1){ // bilinear
                color.g = (texture2D(texture, (pix_loc + vec2(0,1))  / tex_dim).r
        ...
        if (view_mode >= 2){ // demosaicking
            if (demosaicking_mode == 0){ // nearest neighbour
                color.r = texture2D(texture, (pix_loc + vec2(0,1))  / tex_dim).r * ((option_wb) ? white_balance.r : 1.0f);
                color.b = texture2D(texture, (pix_loc + vec2(-1,0)) / tex_dim).r * ((option_wb) ? white_balance.b : 1.0f);
            }
            else if(demosaicking_mode == 1){ // bilinear
                color.r = (texture2D(texture, (pix_loc + vec2(0,1))   / tex_dim).r
                         + texture2D(texture, (pix_loc + vec2(0,-1))  / tex_dim).r ) / 2
                        * ((option_wb) ? white_balance.r : 1.0f);
                color.b = (texture2D(texture, (pix_loc + vec2(-1,0))   / tex_dim).r
                         + texture2D(texture, (pix_loc + vec2(1,0))  / tex_dim).r ) / 2
                        * ((option_wb) ? white_balance.b : 1.0f);
            }
        }
